...
export const incrementTotalPlayers = async () => {
  const statsRef = doc(db, 'stats', 'global');
  await setDoc(statsRef, { totalPlayers: increment(1) }, { merge: true });
};

export const getUserReferralCount = async (userId: string) => {
  const userRef = doc(db, 'users', userId);
  const userSnap = await getDoc(userRef);
  if (userSnap.exists()) {
    return userSnap.data().referralCount || 0;
  } else {
    return 0;
  }
};
